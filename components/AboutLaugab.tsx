...
export function AboutLaugab({ modoPaginaCompleta = false }: AboutLaugabProps) {
  return (
    <section
      ...
      style={
        modoPaginaCompleta ? { minHeight: 220, zIndex: 1 } : { minHeight: 220 }
      }
    >
      ...
      <div className="w-full md:w-1/2 p-4 xs:p-6 md:p-10 flex flex-col gap-3 xs:gap-4">
        <h2
          id="about-laugab-title"
          className="text-xl xs:text-2xl md:text-3xl font-extrabold text-gray-900 mb-2 xs:mb-4"
        >
          Sobre a Laugab Imobiliária
        </h2>
        <p className="text-gray-700 text-sm xs:text-base md:text-lg mb-2 xs:mb-4 leading-relaxed">
          Há mais de <strong>15 anos</strong> no mercado, a Laugab Imobiliária é
          referência em confiança, inovação e atendimento personalizado. Nossa
          equipe já ajudou mais de <strong>2.500 famílias</strong> a encontrar o
          imóvel ideal, com mais de <strong>1.000 imóveis vendidos</strong> e{" "}
          <strong>1.500 imóveis alugados</strong> em todo o Brasil.
        </p>
        <div
          className="grid grid-cols-2 xs:grid-cols-2 md:grid-cols-4 gap-4 xs:gap-6 mb-2 xs:mb-4"
          aria-label="Números da Laugab Imobiliária"
        >
          <div className="flex flex-col items-center min-w-[60px] xs:min-w-[80px]">
            <span className="text-xl xs:text-2xl md:text-3xl font-extrabold text-green-800 drop-shadow-sm">
              15+
            </span>
            <span className="text-[10px] xs:text-[11px] md:text-xs text-green-800 font-medium mt-1">
              anos de experiência
            </span>
          </div>
          <div className="flex flex-col items-center min-w-[60px] xs:min-w-[80px]">
