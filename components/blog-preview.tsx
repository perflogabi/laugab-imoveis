...
// Componente otimizado para imagem do blog
...
// Componente otimizado para metadados do post
const BlogMetadata = ({ category, date }: { category: string; date: string }) => {
  const categoryColor = categoryColors[category] || "bg-gray-600"
  
  return (
    <div className="flex items-center gap-2 mb-3">
      <Badge 
        className={`${categoryColor} ${!categoryColor.includes('text-') ? 'text-white' : ''}`} 
    ...
    <Card className="overflow-hidden">
      ...
      >
        ...
        <CardContent className="p-5">
          <BlogMetadata category={post.category} date={post.date} />
...
export function BlogPreview() {
  // Memoizar os posts para evitar re-renderizações desnecessárias
